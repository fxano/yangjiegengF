...
class _SqflitePageState extends State<SqflitePage> {
  ...
  _getdeleteBtnView(){
    return RaisedButton(
        onPressed: (){
          String sql = "DELETE FROM userInfo";//无条件删除学生表数据
          _delete(_dbName, sql);
        },
  ...
  ///创建数据库db
  _createDb(String dbName,int ver,String dbTables) async {
    //获取数据库路径
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
    print("数据库路径：$path数据库版本$ver");
    //打开数据库
    await openDatabase(
        path,
        version:ver,
  ...
  ///增
  add(String dbName,String sql) async {
    //获取数据库路径
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
    print("数据库路径：$path");
